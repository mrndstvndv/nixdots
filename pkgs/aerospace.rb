# Patched copy of nikitabobko/homebrew-tap's AeroSpace cask.
# Homebrew 6 removed `must_succeed` from install-step `run`.
...
cask "aerospace" do
  ...
  postflight_steps do
    run "/bin/sh", args: ["-c", "/usr/bin/xattr -d com.apple.quarantine \"$1\" 2>/dev/null || true", "sh", "{{staged_path}}/AeroSpace-v#{version}/bin/aerospace"]
    run "/bin/sh", args: ["-c", "/usr/bin/xattr -d com.apple.quarantine \"$1\" 2>/dev/null || true", "sh", "{{appdir}}/AeroSpace.app"]
  end
